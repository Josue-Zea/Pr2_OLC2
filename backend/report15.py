...
def getReport15(file, fileType, countryColumn, departmentColumn, confirmedColumn, daysColumn, countryName, departmentName):
  df = ""
  if fileType == ".csv":
    df = pd.read_csv(file)
    df = df.fillna(0)
  ...
  x1_encoded = le.fit_transform(values[confirmedColumn].to_numpy())
  x2_encoded = le.fit_transform(values[daysColumn].to_numpy())
  
  features  = list(zip(x1_encoded, x2_encoded))
  data = pd.DataFrame(features)
  
  Y = np.asarray(values[confirmedColumn]).reshape(-1,1)
  ...
  plt.title('Tendencia de casos confirmados de Coronavirus en '+departmentName+' de '+countryName+'. \n ' + title, fontsize=10)
  plt.xlabel('Dias')
